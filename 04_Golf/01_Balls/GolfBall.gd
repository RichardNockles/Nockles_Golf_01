...
func _input(event):
    if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
        ...
        print("Mouse Moved to:", current_touch_position)
...
func is_object_clicked(screen_position: Vector2) -> bool:
    ...
    query.from = from
    query.to = to
    query.collision_mask = 5  # "Ground" (2) + "Clickable" (4) = 5
...
func hit_ball():
    var camera = get_viewport().get_camera_3d()
    var hit_direction = -camera.global_transform.basis.z
    hit_direction.y = 0
...
func _process(_delta):
    if is_aiming:
        ...
        var end_point = start_point + direction * hit_power * power_slider.value
        ...
        aiming_line.visible = false # Correctly indented with spaces - FIXED!
        ball_mesh.scale = Vector3(1, 1, 1) # Reset scale when not aiming
